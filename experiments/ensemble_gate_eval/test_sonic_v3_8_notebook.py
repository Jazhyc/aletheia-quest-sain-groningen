"""Pin sonic v3.8: v3.7 linear gate with MAX_CAP rolled back to 4 steps (v3.5's value)."""
...
import build_sonic_v3_8_notebook as builder

SOURCE = REPO_ROOT / "submission/sonic_v3_7.ipynb"

SCORING_CELL = 12
PIPELINE_CELLS = tuple(range(1, 12))
# v3.7's constants except MAX_CAP (which moves back from 12 to 4)
UNCHANGED = ("JUDGE_MARGIN_SD", "BASE_CAP", "PROBE_GAIN",
...
def built(tmp_path_factory) -> dict:
    ...
    if not SOURCE.exists():
        pytest.skip("v3.7 notebook not present")
    output = tmp_path_factory.mktemp("sonic_v3_8") / "sonic_v3_8.ipynb"
...
def test_max_cap_is_four_steps(built):
    got = literals(cell_text(built, SCORING_CELL))
    assert math.isclose(got["MAX_CAP"], 0.41675170554260993, rel_tol=1e-12)
    assert math.isclose(got["MAX_CAP"] / got["BASE_CAP"], 2.0, rel_tol=1e-12)


def test_all_constants_unchanged_except_cap(built, source_notebook):
...
def test_tanh_is_still_gone(built):
...
def test_cap12_is_gone(built):
    executable = "\n".join(line for line in cell_text(built, SCORING_CELL).splitlines()
                           if not line.strip().startswith("#"))
    assert "1.2502551166278297" not in executable
...
def test_confident_judge_not_overturned_by_disagreeing_probe(built):
    """At cap4=0.416, even a 5-sigma probe on disagreement (BASE_CAP=0.208)
    contributes 1.04 judge-z, which can flip a ±1σ judge but not a ±4σ judge."""
    got = literals(cell_text(built, SCORING_CELL))
    judge_z = np.array([4.0, -4.0])
    probe_z = np.array([-5.0, 5.0])
    agreement = (judge_z * probe_z > 0).astype(float)
    cap = got["BASE_CAP"] + agreement * (got["MAX_CAP"] - got["BASE_CAP"])
    combined = judge_z + cap * got["PROBE_GAIN"] * probe_z
    assert combined[0] > 0 > combined[1], "confident judge overturned by disagreeing probe"
