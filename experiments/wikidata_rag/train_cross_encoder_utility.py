...
def serialize_query(row: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Current detector deception score: {score:.4f}"
    )
...
def flatten_batch(
    batch: list[dict[str, Any]], target_name: str
) -> tuple[list[str], list[str], list[torch.Tensor]]:
    queries: list[str] = []
    documents: list[str] = []
    targets: list[torch.Tensor] = []
    for row in batch:
        query = serialize_query(row)
        row_targets = [float(candidate[target_name]) for candidate in row["candidates"]]
...
def train_epoch(
    ...
    *,
    ...
    for batch in loader:
        ...
        queries, documents, target_parts = flatten_batch(batch, target_name)
...
def predict_rows(
    ...
    *,
    ...
    for batch_start in range(0, len(rows), row_batch_size):
        ...
        queries, documents, targets = flatten_batch(batch, "controlled_utility")
        scores = model_scores(
            model, tokenizer, queries, documents, device=device,
            max_length=max_length, sequence_batch_size=sequence_batch_size,
            use_bf16=use_bf16,
...
def checkpoint_key(report: dict[str, Any]) -> tuple[float, float, float, float]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--targets", nargs="+", choices=("utility", "controlled_utility"),
    ...
    for target_name in args.targets:
        ...
        train_candidates = candidate_indices(slices, splits["train"])
        target_scale = max(float(np.std(targets[train_candidates])), 1e-3)
        for learning_rate in learning_rates:
            model, tokenizer = load_model(args.model_name, device)
            dataset = RowDataset(rows, splits["train"])
            ...
            weights = group_weights(rows, splits["train"])
            for epoch in range(1, args.epochs + 1):
                ...
                report.update(metadata | {"training": train_report, "timing": timing})
