...
Cached paired validation improves macro AUROC from `0.95393` to `0.95560`
(`+0.00167`). Instructed remains `0.99813`; varied improves from `0.89500` to
`0.89889`. Unit wins/ties/losses are `4/12/5`, the worst unit loses `0.020`,
...
The NDIF pass scored all 821 test rows in `23.93s` (`34.30` rows/s), with zero
truncation. The frozen blend then regressed macro AUROC from `0.95869` to
`0.95714` (`-0.00155`). Instructed fell from `0.99542` to `0.99375`; varied
fell from `0.90972` to `0.90833`. Unit wins/ties/losses were `3/9/9`, and the
worst unit lost `0.010`.

Reject the ensemble. Its extra score uniqueness (`774 -> 810`) does not
compensate for worse ranking, and the additional base-Qwen inference pass is
not justified. Do not try other blend weights or prompt members on test.
