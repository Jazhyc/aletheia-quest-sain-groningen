#!/bin/bash
#SBATCH --job-name=aq-pid-grpo-e1-test
...
METHOD="pid_grpo_ep1_logits_noreasoning"
METHOD_LOG_DIR="${SHARED_ROOT}/logs/slurm/${METHOD}"
METHOD_LOG_FILE="${METHOD_LOG_DIR}/test-${SLURM_JOB_ID}.out"
...
METHOD_NAME="qwen9b_pid_varied_grpo_ep1_v1"
python "${CODE_ROOT}/experiments/qwen_grpo_lora/evaluate_qwen_grpo_lora_logits.py" \
  --adapter-dir "${SHARED_ROOT}/results/blackbox/${METHOD_NAME}/adapter" \
  --split test \
  --splits-dir "${SHARED_ROOT}/dev_splits" \
  --output-dir \
    "${SHARED_ROOT}/results/blackbox/${METHOD_NAME}/test_logits_empty_reasoning_plain_noreasoning" \
  --batch-size 4 \
