#!/bin/bash
#SBATCH --job-name=aq-hetero-eval
...
python experiments/heterogeneous_adapter_ensemble/evaluate_members.py \
  "${MEMBERS[@]}" --split train --run-name train_heterogeneous_objectives_v1
python experiments/heterogeneous_adapter_ensemble/evaluate_members.py \
  "${MEMBERS[@]}" --split validation --run-name validation_heterogeneous_objectives_v1
