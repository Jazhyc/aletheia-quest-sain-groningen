#!/bin/bash
#SBATCH --job-name=aq-hetero-ngram-test
...
mkdir -p "${LOG_DIR}"
exec >"${LOG_DIR}/ngram-test-${SLURM_JOB_ID}.out" 2>&1
...
python experiments/heterogeneous_adapter_ensemble/evaluate_ngram_stack.py \
  ...
  --evaluation-member deception=results/blackbox/qwen9b_pid_specialist_material_rank1_v1/test_heterogeneous_objectives_v1/generations.jsonl \
  --evaluation-member incorrectness=results/blackbox/qwen9b_heterogeneous_incorrectness_rank1_v1/test_heterogeneous_objectives_v1/generations.jsonl \
  --evaluation-member resolved_intent=results/blackbox/qwen9b_heterogeneous_resolved_intent_rank1_v1/test_heterogeneous_objectives_v1/generations.jsonl \
  ...
  --output "${ENSEMBLE_DIR}/ngram_stack_test_result.json"
