#!/bin/bash
#SBATCH --job-name=aq-pid-q27retry-split
...
mkdir -p "${LOG_DIR}"
exec >"${LOG_DIR}/qwen27-dks-retry-split-${SLURM_JOB_ID}.out" 2>&1
rm -f "logs/slurm/${SLURM_JOB_NAME}-${SLURM_JOB_ID}.bootstrap.out"

WORKTREE_ROOT="${QWEN27_DKS_WORKTREE:?missing QWEN27_DKS_WORKTREE}"
python "${WORKTREE_ROOT}/experiments/privileged_information_distillation/build_qwen27_dks_retry_split.py" \
  ...
  --output-dir results/blackbox/qwen27b_reason_ensemble_dks_member8192_softtrain_retry_v1/retry_split
