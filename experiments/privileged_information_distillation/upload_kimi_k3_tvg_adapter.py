#!/usr/bin/env python3
"""Publish the canonical Kimi K3 soft-distillation LoRA for Phoenix v7."""
...
ADAPTER_DIR = (
    ROOT
    / "results/blackbox/qwen9b_kimi_k3_openrouter_tvg_soft_r16_lr5e5_ep2_v1/adapter"
)
REPOSITORY = "Jazhyc/aletheias-phoenix-v7-kimi-k3-tvg-soft-r16-ep2"
MODEL_CARD = (
    ROOT
    / "experiments/privileged_information_distillation/"
    "kimi_k3_tvg_adapter_model_card.md"
)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="phoenix-v7-kimi-k3-") as temp_dir:
        ...
        commit = api.upload_folder(
            ...
            repo_type="model",
            folder_path=staging,
            commit_message="Upload Phoenix v7 Kimi K3 soft-distillation adapter",
        )
    info = api.model_info(
        REPOSITORY,
        revision=commit.oid,
