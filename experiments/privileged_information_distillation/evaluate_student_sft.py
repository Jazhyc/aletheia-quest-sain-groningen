...
def load_records(split: str, splits_dir: Path, config: dict[str, Any], tokenizer: Any) -> pd.DataFrame:
...
def main() -> None:
    ...
    records = load_records(args.split, args.splits_dir.resolve(), first, tokenizer)
    ...
    for lora_id, (adapter_dir, config) in enumerate(zip(adapter_dirs, configs, strict=True), 1):
        ...
        output_dir = adapter_dir.parent / args.split
