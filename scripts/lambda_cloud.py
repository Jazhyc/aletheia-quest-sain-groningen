    ...
    def request(
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "aletheias-quest-lambda-runner/1",
            },
        )
        try:
...
def require_campaign_instance(
    ...
    *,
    ...
    if require_active and (
        ...
        fail(
            f"campaign {campaign!r} is {instance.get('status')!r}, not ready for SSH"
        )
    return instance
...
def default_private_key_path() -> Path:
    ...
    return Path(configured).expanduser() if configured else Path.home() / ".ssh/id_ed25519"
...
def command_types(args: argparse.Namespace, client: LambdaCloudClient) -> None:
    ...
    for name, item in client.instance_types().items():
        ...
        if args.available_only and not row["regions"]:
            continue
        if args.min_gpu_memory and extract_gpu_memory_gib(row["gpu"]) < args.min_gpu_memory:
            continue
        rows.append(row)
    rows.sort(key=lambda row: (not bool(row["regions"]), row["usd_per_hour"], row["name"]))
    print_json(rows)
...
def command_launch(args: argparse.Namespace, client: LambdaCloudClient) -> None:
    ...
    if existing:
        ...
        return
    if not args.yes:
        fail("launch creates a billable GPU instance; pass --yes after reviewing the price")
    ...
    if args.region not in row["regions"]:
        fail(
            f"{args.instance_type} currently has no reported capacity in {args.region}; "
    ...
    payload: dict[str, Any] = {
        ...
        "name": campaign_name,
        "tags": [
            {"key": "project", "value": "aletheias-quest"},
    ...
    print_json(
        {
            ...
            "excluded": ["LAMBDA_API_KEY", "NDIF_API_KEY"],
        }
    )
...
def add_private_key_argument(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="local SSH private key (default: LAMBDA_SSH_KEY_PATH or ~/.ssh/id_ed25519)",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    types_parser = subparsers.add_parser("types", help="list GPU types and live capacity")
    ...
    wait_parser = subparsers.add_parser("wait", help="wait until a campaign is SSH-ready")
